...
class BlockPolling {
  ...
  async _poll(fromBlock) {
    try {
      const latestBlock = (await this._web3.eth.getBlockNumber()) - this._confirmations;

      fromBlock = fromBlock || latestBlock;

      const toBlock = Math.min(fromBlock + this._chunkSize - this._confirmations - 1, latestBlock);
      ...
      const nextBlock = Math.max(fromBlock, toBlock + 1);
